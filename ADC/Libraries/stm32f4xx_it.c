...
void EXTI0_IRQHandler(void) {
    ...
    if (EXTI_GetITStatus(EXTI_Line0) != RESET) {
      ...
      //while(GPIO_ReadInputDataBit(GPIOD, GPIO_Pin_0) == 0x00);

         while(1)
          {
            //timperiod = TIM_PERCT/Den;
            if (TIM_GetCounter(TIM2) < 1000){
              TIM4 -> CCR1 = TIM4_PER/2;
            }
            else{
              TIM4 -> CCR1 = 0;
            }
...
void SPI2_IRQHandler(void)
{
  if(SPI_I2S_GetITStatus(SPI2, SPI_I2S_IT_RXNE) != RESET)
  {
    ...
    if(SPI_SLAVE_Buffer_Rx[0] == 0x20)
    {
      SPI_SLAVE_Buffer_Tx[S_Rx] = SPI_SLAVE_Buffer_Rx[S_Rx];
      S_Rx++;
      if(SPI_SLAVE_Buffer_Rx[31] == 0x3F)
      {
        Flag_end = 1;
      }
    }
